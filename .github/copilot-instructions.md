...
**Note**: The project is configured to use bun (see `packageManager` in `package.json`). If you prefer npm, you can still use it, but bun is the standard for this project.
...
- **CI Testing**: Use `bun run test:ci` in CI to generate coverage reports; use `bun test` for faster local development
...
- Run `bun test` to ensure spec compliance
- **ALWAYS run `bun run check` before considering a change "done"** — This verifies tests, linting, and formatting all pass
- **Formatting is required** — All code must pass `bun run format:check` before committing. Run `bun run format` to auto-fix formatting issues.
...
- **Maintain 100% code coverage** — When adding new code, always add corresponding test cases. Use `bun run test:ci` to verify coverage remains at 100%. If coverage drops, add tests to cover the new code paths.
...
- **Only update `bun.lockb` when adding, removing, or updating dependencies** — Do not commit incidental changes to bun.lockb that occur from running `bun install`. If you need to run these commands for testing, revert any unintended bun.lockb changes before committing.
...
- **Run performance tests first** — Use `bun run perf` to identify actual bottlenecks before making changes
...
- Run `bun run perf` to benchmark all functions
